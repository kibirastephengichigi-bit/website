...
from database import db, UserDatabase
from security import verify_password, verify_totp, generate_session_token, generate_csrf_token, hash_csrf_token, verify_csrf_token

# Optional: Use Go telemetry service for better performance
USE_GO_TELEMETRY_SERVICE = os.getenv("USE_GO_TELEMETRY_SERVICE", "false").lower() == "true"
GO_TELEMETRY_SERVICE_URL = os.getenv("GO_TELEMETRY_SERVICE_URL", "http://localhost:9002")

if USE_GO_TELEMETRY_SERVICE:
    try:
        from go_services_client import TelemetryServiceClient
        _telemetry_service = TelemetryServiceClient(base_url=GO_TELEMETRY_SERVICE_URL)
    except ImportError:
        _telemetry_service = None
        USE_GO_TELEMETRY_SERVICE = False
else:
    _telemetry_service = None

def record_telemetry(event_type: str, endpoint: str = "", message: str = "", success: bool = True) -> None:
    """Record a telemetry event if the Go service is available"""
    if USE_GO_TELEMETRY_SERVICE and _telemetry_service:
        try:
            _telemetry_service.record_event(event_type, endpoint, message, success)
        except Exception:
            pass  # Silently fail if telemetry service is unavailable
...
class AdminRequestHandler(BaseHTTPRequestHandler):
  ...
  def _json_response(self, status: int, payload: dict[str, Any]) -> None:
    ...
    etag = self._generate_etag(body.decode('utf-8'))
  ...
  def _get_current_user(self) -> dict[str, Any] | None:
    ...
    session = db.get_session(token, ip_address=ip_address, user_agent=user_agent)
    if not session:
      return None
  ...
  def do_GET(self) -> None:
    ...
    if path == "/api/csrf-token":
      ...
      csrf_secret = config.DEFAULT_ADMIN_TOTP_SECRET or "default-csrf-secret"
      csrf_hash = hash_csrf_token(csrf_token, csrf_secret)
      
      # Store the CSRF token hash in the database
      user_data = db.get_user_by_username(user["username"])
      if user_data:
        db.create_csrf_token(user_data["id"], csrf_hash)
      
      self._json_response(HTTPStatus.OK, {"csrf_token": csrf_token})
      return
    ...
    if path == "/api/session/refresh":
      ...
      if session:
        ...
        new_expires = datetime.now(timezone.utc) + timedelta(hours=config.DEFAULT_SESSION_HOURS)
        
        import sqlite3
        with sqlite3.connect(db.db_path) as conn:
          conn.execute("""
            UPDATE user_sessions SET expires_at = ? WHERE token = ?
          """, (new_expires.isoformat(), session["token"]))
          conn.commit()
        
        self._json_response(HTTPStatus.OK, {"expires_at": new_expires.isoformat()})
      else:
        self._json_response(HTTPStatus.UNAUTHORIZED, {"error": "No active session"})
      return
    ...
    if path == "/api/session/revoke":
      ...
      session = user.get("session")
      if session:
        db.destroy_session(session["token"])
        self._json_response(HTTPStatus.OK, {"message": "Session revoked"})
      else:
        self._json_response(HTTPStatus.UNAUTHORIZED, {"error": "No active session"})
      return

    if path == "/api/session/revoke-all":
      # Revoke all sessions for the current user
      user_data = db.get_user_by_username(user["username"])
      if user_data:
        db.revoke_all_user_sessions(user_data["id"])
        db.revoke_all_csrf_tokens(user_data["id"])
        self._json_response(HTTPStatus.OK, {"message": "All sessions revoked"})
      else:
        self._json_response(HTTPStatus.UNAUTHORIZED, {"error": "User not found"})
      return
    ...
    if path == "/api/session/list":
      ...
      user_data = db.get_user_by_username(user["username"])
      if user_data:
        sessions = db.list_user_sessions(user_data["id"])
        # Don't expose tokens
        for session in sessions:
          session.pop("token", None)
        self._json_response(HTTPStatus.OK, {"sessions": sessions})
      else:
        self._json_response(HTTPStatus.UNAUTHORIZED, {"error": "User not found"})
      return
  ...
  def do_POST(self) -> None:
    ...
    if path == "/api/media":
      if self.command == "GET":
        ...
        # Handle file upload (multipart form data) or JSON payload
        ...
        if "multipart/form-data" in content_type:
          ...
          file_content_type = getattr(file_field, "type", "") or ""
          ...
          if file_content_type not in ALLOWED_UPLOAD_TYPES:
          ...
          media_type = 'image' if file_content_type.startswith('image/') else 'video' if file_content_type.startswith('video/') else 'document'
          media_data = db.create_media(
            filename=item.get("filename", file_name),
          ...
          self._json_response(HTTPStatus.CREATED, {"item": media_data})
          return
        else:
          # Handle JSON payload (create media record from existing URL)
          try:
            payload = self._read_json_body()
        ...
        media = db.get_media_by_id(int(media_id))
        if media:
          # Delete file from storage if it exists
          if media.get("url"):
            file_path = media["url"].replace("/uploads/admin/", "")
            storage_path = config.UPLOADS_DIR / file_path
...
def main() -> None:
  ...
  # Revoke all active sessions on server restart for security
  revoked_count = db.revoke_all_sessions()
  if revoked_count > 0:
    print(f"🔒 Revoked {revoked_count} active session(s) on server restart")
