...
if [ ! -f "$PID_FILE" ]; then
    echo -e "${YELLOW}No PID file found. Frontend may not be running.${NC}"
    ...
    if ss -tuln | grep -q ":3000 "; then
        echo -e "${YELLOW}Port 3000 is still in use. Force killing...${NC}"
...
if ! ps -p $PID > /dev/null 2>&1; then
    echo -e "${YELLOW}Process $PID not found. Removing stale PID file.${NC}"
    rm "$PID_FILE"
    
    NEXT_PIDS=$(pgrep -f "next dev" || true)
    if [ -n "$NEXT_PIDS" ]; then
        echo -e "${YELLOW}Found remaining Next.js processes: $NEXT_PIDS${NC}"
        echo -e "${YELLOW}Stopping them...${NC}"
        echo "$NEXT_PIDS" | xargs kill 2>/dev/null || true
    fi
...
NEXT_PIDS=$(pgrep -f "next dev" || true)
if [ -n "$NEXT_PIDS" ]; then
    echo -e "${YELLOW}Force killing remaining Next.js processes...${NC}"
