...
export function InteractiveHeroSection() {
  ...
  const eyebrow = heroData?.eyebrow || siteContent.hero.eyebrow;
  const headline = heroData?.headline || siteContent.hero.headline;
  const description = heroData?.description || "";
  const badges = heroData?.badges ? JSON.parse(heroData.badges) : ["Licensed Psychologist", "Senior Lecturer", "Research Leader"];
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-100">
                {badges.map((badge: string, index: number) => (
                  <span key={index} className="inline-flex items-center rounded-full border border-emerald-500/30 bg-gradient-to-r from-emerald-50 to-teal-50 px-4 py-2 text-xs font-bold text-emerald-700 shadow-sm hover:shadow-md transition-all duration-200">
                    {badge}
                  </span>
                ))}
              </div>
